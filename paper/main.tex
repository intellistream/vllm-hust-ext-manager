...
\input{generated/evidence-summary.tex}
\raggedbottom
\setlength{\emergencystretch}{2em}
\def\UrlBreaks{\do\/\do-\do\_}
\sloppy
\hbadness=10000
\title{\Large \bf ECPA: Extension Contracts and Transactional Activation for LLM Inference Systems}
...
immutable execution plan; a transactional runtime prepares, activates,
observes, exposes, and rolls back that plan. Authority-owned evidence discharges
plan obligations, and an ExposureGate makes a generation externally visible
only after fresh evidence covers its epoch-scoped targets. We derive the model
from a source-bound corpus of \ECPACorpusExtensions{} extensions and
\ECPAAdaptationCandidates{} adaptation candidates spanning scheduling, KV data
paths, deployment control, and observability.  We implement a host-neutral
...
compensation.  ECPA provides explicit extension semantics beyond API
availability while keeping host bindings and optimization-specific algorithms
outside the contract runtime.
\end{abstract}
...
fail-closed formal harness are implemented.  The source-bound static corpus is
measured; the matched prototype-host experiment and performance evaluation
remain incomplete.  We do not present modeled decisions, interface fixtures,
or provider demonstrations as proof of runtime effectiveness.
...
\cite{vllm-security}.
For the evaluation harness specifically, the experiment host, runner,
registered lifecycle sources, and registry-pinned generic observer are trusted
evidence components; the SUT may fail but is not modeled as same-user code
...
The runner sends challenge-bound phase commands carrying plan, launch,
controller-instance, and invocation identities, and binds Linux PID/start-time
identities, exact argv, and exact source datagram bytes.  An acknowledgement is
only control-plane progress and is never treated as proof that a workload,
fault, hook, or policy effect occurred.  Formal lifecycle facts instead come
from five separately registered commands: a readiness probe, workload driver,
fault actuator, host observer, and process monitor.  Each source receives its
own control pipe and kernel-credentialed Unix datagram channel and may emit one
canonical receipt bound to the current challenge, sequence, execution identity,
value, time, and observed SUT process.  The host observer owns only effect
capture from the prototype host's native journal; it does not stand in for
readiness, workload, fault, or shutdown authority.  A formal observable carrying
\code{causal\_ack} is rejected.  The generic observer derives activation path,
effective claim, invocation, and coverage observations from the host-owned
event stream, but receives no phase message in a formal-real run.  The SUT is
not given a telemetry path whose self-reported contents can be relabeled as
trusted observation.

Controlled fixture services are explicitly classified as interface-only and
cannot produce a formal manifest or enter the paper aggregate.  The production
adapter registry is empty until the real host seam, sources, and exact command
fingerprints are reviewed, so no formal-real result is currently claimed.  The
runner double-reads process start ticks around exact-argv capture to reject
PID-reuse races, and validates the datagram's kernel PID/UID/GID against the
registered source before committing its receipt.
Known fixture artifacts and textual interpreter references are excluded both at
launch and offline validation; this remains a trusted-registry boundary, not a
sandbox for malicious registered code.  Manifest ingestion opens every path
component without following symlinks and hashes the same bytes it parses.
...
The argument covers admission safety, not arbitrary functional correctness.
If any authorized obligation source---host, provider, or external
authority---emits a semantically false event, or an effect outside the declared
taxonomy matters, the premises do not hold.  Similarly,
rollback preserves exposure safety by closing the candidate before restoring a
predecessor, but cannot guarantee recovery of external state that the manager
does not own.  These boundaries determine the observer-independence and
failure-injection experiments rather than being treated as implementation
details.
...
One major evaluation piece remains incomplete.  The implementation now has a
...
machine-readable taxonomy and independently labeled modeled oracle now measure
the static admission mechanics, with the limitations reported in
Section~\ref{sec:evaluation}.  Process-owned evidence still needs a completed
real launch and aggregation experiment across parent/worker topologies.
Consequently we claim measured modeled L2 admission, not a completed end-to-end
\system evaluation.

To reduce the remaining host-integration risk without promoting a synthetic
run into a result, the artifact also includes a cross-repository adapter
preflight.  Given an exact clean prototype-host commit and tree, it binds the host
producer's source blobs and native engine/worker rank assignments, freezes
the checked Git-object bytes against worktree drift, executes that tree's real
preemption-policy controller, persists its resolution and
dispatch through the deployment-owned journal, and translates the exact
dispatch bytes into a Plan-bound invocation statement.  This establishes wire
and adapter-path compatibility only: it starts no serving engine, proves no
worker coverage, and is schema-forbidden from claiming a formal-real result.
...
test keys; a production host issuer, trust root/key lifecycle, and
...
resulting machine-readable corpus contains \ECPACorpusExtensions{} registered
extensions and \ECPAAdaptationCandidates{} adaptation candidates.  Candidates
contain relevant hook, adapter, policy, or
...
The corpus deliberately crosses five semantic families: scheduler decisions,
KV transfer and materialization, state/cache management, deployment control,
and lifecycle observability (Figure~\ref{fig:case-coverage}).  BidKV is one scheduler-policy instance.  It does
...
Prefix state cache & request-state reuse & cache lookup/use event and state-ownership boundary \\
Tiered KV cache & state placement and movement & data-path invocation and tier ownership \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure*}[t]
\centering
\begin{tikzpicture}[x=2.85cm,y=0.58cm,font=\footnotesize]
\foreach \x/\name in {0/{Control / parent},1/{Engine / scheduler},2/{Worker / data path},3/{External authority}}
  \node[align=center,font=\bfseries\footnotesize] at (\x,1.0) {\name};
\foreach \y/\name in {0/BidKV, -1/Mooncake, -2/{Production stack}, -3/{KV admission}, -4/{Lifecycle profiler}, -5/{Prefix state cache}, -6/{Tiered KV cache}}
  \node[anchor=east] at (-0.58,\y) {\name};

\draw[fill=ecpablue] (0.68,-0.27) rectangle (1.32,0.27);
\draw[fill=ecpablue] (1.68,-1.27) rectangle (2.32,-0.73);
\draw[fill=ecpablue] (2.68,-1.27) rectangle (3.32,-0.73);
\draw[fill=ecpablue] (-0.32,-2.27) rectangle (0.32,-1.73);
\draw[fill=ecpablue] (2.68,-2.27) rectangle (3.32,-1.73);
\draw[fill=ecpablue] (0.68,-3.27) rectangle (1.32,-2.73);
\draw[fill=ecpablue] (-0.32,-4.27) rectangle (0.32,-3.73);
\draw[fill=ecpablue] (0.68,-4.27) rectangle (1.32,-3.73);
\draw[fill=ecpablue] (1.68,-4.27) rectangle (2.32,-3.73);
\draw[dashed,thick,fill=ecpablue!35] (0.68,-5.27) rectangle (1.32,-4.73);
\draw[dashed,thick,fill=ecpablue!35] (1.68,-5.27) rectangle (2.32,-4.73);
\draw[dashed,thick,fill=ecpablue!35] (1.68,-6.27) rectangle (2.32,-5.73);

\node[anchor=west] at (-0.30,-7.05) {\tikz\draw[fill=ecpablue] (0,0) rectangle (.18,.18); selected case};
\node[anchor=west] at (1.45,-7.05) {\tikz\draw[dashed,thick,fill=ecpablue!35] (0,0) rectangle (.18,.18); corpus pressure only};
\end{tikzpicture}
\caption{The evaluation is multi-extension even though it uses one prototype
host.  Selected cases exercise different process and authority domains;
dashed rows shape the contract taxonomy but are not promoted to formal runtime
results.  Empty cells mean the case does not claim that authority, not that the
corpus was unexamined.}
\label{fig:case-coverage}
\end{figure*}

The cases exercise distinct failure semantics.  A KV connector can be
configured while a worker has the wrong connector or wheel variant.  A
deployment provider can render valid inputs while an external rollout is
absent, and two controllers can conflict over replicas.  A lifecycle observer
can run in the parent while one worker emits no events.  A cache/tiering
extension can execute code without changing placement or reuse.  A scheduler
policy, including BidKV, can be selected without ever affecting a scheduling
decision.  For each, the corpus records resources, process topology, external
...
The corpus supplies breadth across \ECPACorpusExtensions{} registered extensions,
\ECPAAdaptationCandidates{} adaptation candidates, multiple plugin classes, and
distinct process/authority scopes.
...
For the L2 study, we freeze a canonical resource-and-alias taxonomy,
...
\ECPACorpusExtensions{} registered
extensions and add explicitly marked synthetic controls for otherwise absent
...
recall 0.  This result establishes conformance only to the small modeled static
corpus.  Every conflict case contains a synthetic boundary control, so the
result does not establish real-plugin conflict prevalence, runtime
effectiveness, or external validity.
...
so activation, rollback, and performance metrics remain null; the modeled L2
result above is separate.  A \ECPAReferenceStarts{}-start helper-process run tests the
...
\ECPAFormalArms{} arms and \ECPAFirstStudyStarts{} independent starts.  It
remains pre-admission while the producer, deployment identity, commands, and
adapter registry are not all frozen; preregistration is not a result.
...
\midrule
Static planning & \ECPAPlannerCases{} labeled cases: \ECPAPlannerAdmits{} admit,
\ECPAPlannerRejects{} reject & modeled \\
Source binding & \ECPASourceRepositories{} repositories and
\ECPASourceObjects{} Git objects & observed \\
Namespace safety & installed entry: expected 1; changed lookup: 0 & negative \\
Transaction model & \ECPATransactionModelStates{} states,
\ECPATransactionModelEdges{} valid edges; no abstract counterexample & modeled \\
Formal-real & \ECPAFormalCompleted{} complete, \ECPAFormalPlanned{} planned;
\ECPAVerifiedAdapters{} registered adapters & planned \\
Rollback & reference faults only; real injection pending & planned \\
Overhead & matched \ECPAFormalArms{}-arm experiment & planned \\
\bottomrule
\end{tabularx}
\end{table}
...
\centering
\small
\caption{Authority chain for the first formal-real cell.  Each row is a
different fact; later rows cannot infer earlier ones.}
\begin{tabularx}{\columnwidth}{@{}lXX@{}}
\toprule
Artifact & Establishes & Current state \\
\midrule
Study preregistration & arms, order, falsifier, outcomes & frozen; no result \\
Producer receipt & reviewed evidence seam merged into prototype host & absent \\
Deployment packet & matched identities and reviewed commands &
\ECPADeploymentBlockers{} blockers \\
Runtime registry & exact commands permitted to execute &
\ECPAVerifiedAdapters{} adapters \\
Run generation & raw observations for one independent start & 0 of
\ECPAFirstStudyStarts{} starts \\
\bottomrule
\end{tabularx}
\end{table}
...
At the incident's pre-fix baseline, 66 CPU tests pass.  On commit
...
The source-bound modeled L2 corpus additionally contains \ECPAPlannerCases{}
frozen composition cases over \ECPACorpusExtensions{} registered extensions.
The compiler matches all \ECPAPlannerAdmits{} admits and \ECPAPlannerRejects{}
rejects, including reject classes, after independent oracle review.  Exact
repository commits, root trees, and \ECPASourceObjects{} cited Git objects
across \ECPASourceRepositories{} repositories
bind the corpus to inspected source.  These numbers measure conformance to the
modeled taxonomy, not real conflict prevalence or runtime effectiveness.

All runtime and performance quantities remain planned.  Table and figure
generators must refuse to replace missing values with zero.  The final paper
will include per-cell confidence intervals and raw failure counts.  Until
H1--H4 are measured, the strongest defensible runtime claim is that the
architecture makes evidence obligations explicit and the harness fails closed;
it does not establish production effectiveness.
...
\textbf{Consistent update and dynamic upgrade.}  Consistent network updates
use versioned configurations and controlled exposure to prevent packets from
observing invalid intermediate network states \cite{reitblatt-update}.  ECPA
shares the concern for externally visible intermediate states, but its unit of
consistency is an inference-extension plan whose obligations span host
processes, providers, and external authorities; it does not provide per-packet
network-update semantics.  Dynamic software update systems such as Kitsune
make update points and state transformation explicit while preserving a
running program \cite{kitsune}.  ECPA neither rewrites live code nor proves
state-transformer correctness.  It coordinates separately installed
extensions and refuses exposure when their declared cross-process effects lack
fresh evidence.
...
independent host and extension repositories; submit an upstream RFC and incorporate public
...
across multiple extension classes, process roles, versions/topologies, and injected
...
paper nor the \ECPACorpusExtensions{}+\ECPAAdaptationCandidates{} corpus
