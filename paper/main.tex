...
\begin{abstract}
Extensible LLM inference systems can report a plugin as enabled even when the
selected implementation never affects one or more serving processes.  We call
this a false-effective outcome.  ECPA prevents such claims by treating
activation as an evidence-bound transaction rather than a package-loading
event.  A transaction combines an immutable activation plan, an epoch-scoped
target-process set, authority-scoped fresh evidence, an ExposureGate that
withholds traffic eligibility until obligations hold, and a durable
coordinator that preserves intent and a predecessor for recovery.  ECPA keeps
installed, configured, enabled, and runtime-effective as separate predicates;
neither import success nor control-plane acknowledgement proves worker-side
effect.  We ground the design in a corpus of 11 registered extensions and five
adaptation candidates spanning schedulers, KV connectors, lifecycle observers,
and deployment adapters.  The current Python prototype validates manifests,
plans, resource ownership, durable generations, replay fencing, crash recovery,
and fail-safe rollback through tests and adversarial traces.  An
installed-package experiment also reproduces a namespace regression in which
all legacy unit tests pass while the only installed plugin becomes
undiscoverable.  These results establish a concrete false-effective failure
and validate coordinator safety properties, but do not yet establish
production latency, throughput, or multi-host availability.  We therefore
state performance and live-runtime campaigns as planned work.
\end{abstract}
