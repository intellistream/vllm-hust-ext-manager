...
positive quorum threshold.  Runtime effectiveness is:
\begin{align*}
...
identities, exact argv, and exact observer-pipe bytes.  An acknowledgement is
only control-plane progress and is never treated as proof that a workload,
fault, hook, or policy effect occurred.  Formal facts must come from a
registry-pinned observer reading a vLLM-HUST-host-owned event channel; the SUT
is not given a telemetry file whose self-reported contents can be relabeled as
trusted observation.  Controlled fixture services are explicitly classified
as interface-only and cannot produce a formal manifest or enter the paper
aggregate.  The production adapter registry is empty until the real host seam,
observer, and exact command fingerprints are reviewed, so no formal-real result
is currently claimed.  Each observer event must match the phase's ACK result,
challenge, sequence, and invocation identity.  The runner double-reads process
start ticks around exact-argv capture to reject PID-reuse races, while the
observer independently reports the SUT process identity it observed.
