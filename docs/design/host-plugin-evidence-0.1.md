...
The runtime host, not plugin code, observes the existing entry-point path. The
...
`(process_epoch, event, entry-point group, entry-point name)`.

The host supplies hostname, role, ordinal, PID, Linux process start identity,
integer process epoch, and observation time. Plan and launch IDs are injected
