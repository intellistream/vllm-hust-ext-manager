...
identity, scenario/protocol digests, arm, repetition, and order. A content-chain
artifact binds exact command, sanitized environment, raw observations, raw
stdout/stderr, scenario, protocol, intake, and independently recomputed oracle.
...
interfaces. It can generate raw formal records from supplied commands, but the
checked matrix stays planned because no real vLLM commands or frozen non-null
formal identity were supplied. Ready, fault, and shutdown times come only from
raw observer events; startup is launch-to-ready.
