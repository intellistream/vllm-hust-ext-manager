...
from \ECPACorpusExtensions{} extensions and
\ECPAAdaptationCandidates{} adaptation candidates in vLLM-HUST, and implement
a reference coordinator and adversarial evaluation harness covering namespace
errors, resource conflicts, partial worker coverage, replay, crash recovery,
lease expiration, and failed compensation. ECPA provides explicit extension
semantics beyond API availability while preserving optimization-specific
algorithms outside the runtime.
\end{abstract}
...
measured; the real vLLM-HUST matched experiment and performance evaluation
...
capture from the vLLM-HUST-host-owned journal; it does not stand in for
...
that a real vLLM-HUST deployment recovers correctly.  H3 therefore remains a
...
its fault-injection suite.  Establishing them for vLLM-HUST requires the
...
terminal or compensating paths.  Exposure is distinct from activation: a
...
preflight.  Given an exact clean vLLM-HUST commit and tree, it binds the host
producer's source blobs and native EngineCore/worker rank assignments, freezes
...
Producer receipt & reviewed host seam merged into vLLM-HUST & absent \\
