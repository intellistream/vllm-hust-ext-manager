...
from 11 extensions and five adaptation candidates in vLLM-HUST, and implement
...
For the L2 study, we freeze a canonical resource-and-alias taxonomy, 31 modeled
single/pair/triple compositions, and a separately stored manual candidate
oracle before implementing the evaluator.  The models cover all 11 registered
extensions and add explicitly marked synthetic controls for otherwise absent
conflict boundaries; the five adaptation candidates remain excluded.  The
oracle passed independent exact-digest label review.  This preregistration is
evaluated after review: the reference planner matches all 21 admits and 10
rejects, including reject error codes.  Reject precision, reject recall, and
admit recall are each 1.0; a reject-all baseline has precision $10/31$ and admit
...
result above is separate.  A 45-start helper-process run tests the
...
\midrule
Static planning & 31 labeled cases: 21 admit, 10 reject & modeled \\
Source binding & 15 repositories and 41 Git objects & observed \\
Namespace safety & installed entry: expected 1; changed lookup: 0 & negative \\
Formal-real & 0 complete, 30 planned; registry empty & planned \\
Rollback & reference faults only; real injection pending & planned \\
Overhead & matched three-arm experiment & planned \\
\bottomrule
\end{tabularx}
\end{table}
...
The source-bound modeled L2 corpus additionally contains 31 frozen composition
cases over 11 registered extensions.  The compiler matches all 21 admits and
10 rejects, including reject classes, after independent oracle review.  Exact
repository commits, root trees, and 41 cited Git objects across 15 repositories
